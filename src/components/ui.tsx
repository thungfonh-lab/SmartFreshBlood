...
const GROUP_COLORS: Record<BloodGroup, string> = {
  ...
  "B-": "bg-emerald-200 text-emerald-800",
...
export function FreshScoreBar({ score }: { score: number }) {
  ...
  const textColor = { fresh: "text-emerald-600", medium: "text-amber-600", low: "text-red-600" }[level];
  return (
...
export function SuccessBox({ message }: { message: string }) {
  return <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-sm text-emerald-700">{message}</div>;
}
